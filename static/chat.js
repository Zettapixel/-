...
const connect = () => {
    ...
    socket = new WebSocket(
        'ws://localhost:8080/?' + encodeURIComponent(userName)
    );
    ...
    socket.addEventListener('message', (msg) => {
        ...
        if (obj.type === 'message') {
            if (obj.data === '@bot date') {
                const d = Date();
                document.getElementById('fromServer').innerHTML += `${d}<br />`;
            } else {
                document.getElementById(
                    'fromServer'
                ).innerHTML += `<b>${obj.name}</b>: ${obj.data}<br />`;
            }
        } else if (obj.type === 'enter') {
            document.getElementById(
                'fromServer'
            ).innerHTML += `${obj.name}が入室しました！<br />`;
        } else if (obj.type === 'leave') {
            document.getElementById(
                'fromServer'
            ).innerHTML += `${obj.name}が退室しました！<br />`;
        }
    });
...
// 入室・退室ボタン
document
    .getElementById('enterLeaveButton')
    .addEventListener('click', enterLeaveRoom);
